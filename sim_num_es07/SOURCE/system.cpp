...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    int spin_l = _particle(this->pbc(i-1)).getspin();
    int spin_r = _particle(this->pbc(i+1)).getspin();

    double h = _J * (spin_l + spin_r) + _H;
    double p_u = 1 / (1 + exp(-2 * _beta * h));
    if(_rnd.Rannyu()<p_u){
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      for(int i=0; i<_npart; i++){ 
        ...
        if(_rnd.Rannyu() > 0.5) _particle(i).flip(); // to randomize the spin configuration (temperatura infinita, se cancello questa riga ho il g.s. con tutti gli spin up)
      }
...
//    }
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setvelocity(int(_rnd.Rannyu(0.0, 3.0)), sign * sqrt(3.0 * _temp));
    }
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8.0 * M_PI * _rho *(1 / (9 * pow(_r_cut, 9)) - 1 / (3 * pow(_r_cut, 9))); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = 32 * M_PI * (1 / (9 * pow(_r_cut, 9)) - 1 / (6 * pow(_r_cut, 3))); // TO BE FIXED IN EXERCISE 7
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4 => la correzione che ho fatto è stata aggiungere una radice alla temperatura
        _measure_pofv = true;
...
/*void System :: initialize_fcc_halfbox(){
  double L= _side(0) / 2.0;
  int ncell = 1;
  while(4 * ncell * ncell * ncell < _npart) ncell++;
  double a = L / ncell; //lato di ogni cella
  int ip = 0;
  for (int ix = 0; ix < ncell; ix++) {
    for (int iy = 0; iy < ncell; iy++) {
      for (int iz = 0; iz < ncell; iz++) {
        ...
        if (ip < _npart) {
            _particle(ip).setposition(0, x0);
            _particle(ip).setposition(1, y0);
            _particle(ip).setposition(2, z0);
            _particle(ip).acceptmove();
            ip++;
        }

        if (ip < _npart) {
            _particle(ip).setposition(0, x0);
            _particle(ip).setposition(1, y0 + 0.5 * a);
            _particle(ip).setposition(2, z0 + 0.5 * a);
            _particle(ip).acceptmove();
            ip++;
        }

        if (ip < _npart) {
            _particle(ip).setposition(0, x0 + 0.5 * a);
            _particle(ip).setposition(1, y0);
            _particle(ip).setposition(2, z0 + 0.5 * a);
            _particle(ip).acceptmove();
            ip++;
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    for(int i=0; i<_npart; i++){
      double v = sqrt (pow(_particle(i).getvelocity(0), 2) + pow(_particle(i).getvelocity(1), 2) + pow(_particle(i).getvelocity(2), 2));
      if(v < _n_bins_v*_bin_size_v){
...
// TO BE FIXED IN EXERCISE 6
  if(_measure_magnet){
    int tot_spin = 0;
    for(int i=0; i<_npart; i++){
      tot_spin += _particle(i).getspin();
    }
    _measurement[_index_magnet] = (double)tot_spin / (double)_npart; // usa come energia U/N qui ho scritto M / N
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
    if(!_measure_tenergy){
    cerr << "PROBLEM: SPECIFIC_HEAT requires TOTAL_ENERGY" << endl;
    exit(EXIT_FAILURE);
    }
    double s_i, s_j;
    double e=0.0;
    for (int i=0; i<_npart; i++){
      s_i = double(_particle(i).getspin());
      s_j = double(_particle(this->pbc(i+1)).getspin());
      e += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
    }

    double u = e / double(_npart);
    _measurement[_index_cv] = u * u;
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if(_measure_chi){
    int tot_spin = 0; //per non ripetere il calcolo potrei mettere un if
      for(int i=0; i<_npart; i++){
        tot_spin += _particle(i).getspin();
      }
    _measurement(_index_chi) = (double)(tot_spin * tot_spin) / (double)_npart ;  // chi/N
  }
...
void System :: averages(int blk){
  ...
  if(_measure_gofr){
    coutf.open("OUTPUT/gofr_MD.dat",ios::app);
    for(int i=0; i<_n_bins; i++){
      double lower = i * _bin_size;
      double center = lower + 0.5 * _bin_size;
      double upper = lower + _bin_size;

      double norm = (4.0 / 3.0) * M_PI * (pow(upper, 3) - pow(lower, 3)) * _rho * _npart;
      double average     = _average(_index_gofr + i) / norm;
      double sum_average = _global_av(_index_gofr + i) / norm;
      double sum_ave2    = _global_av2(_index_gofr + i) / (norm * norm);

      coutf << setw(12) << center
            << setw(12) << average
            << setw(12) << sum_average / double(blk)
  ...
  if(_measure_pofv){
    ...
    for(int i=0; i<_n_bins_v; i++){
      _average(_index_pofv + i);
      double center = (i + 0.5) * _bin_size_v;
      double norm = double(_npart) * _bin_size_v;
      double average     = _average(_index_pofv + i) / norm;
      double sum_average = _global_av(_index_pofv + i) / norm;
      double sum_ave2    = _global_av2(_index_pofv + i) / (norm * norm);
  ...
  if(_measure_cv){
    ...
    average = _beta * _beta * double(_npart) * (_average(_index_cv) - pow(_average(_index_tenergy), 2));
    sum_cv += average;
    sum_cv2 += average * average;
    coutf << setw(12) << blk
        << setw(12) << average
        << setw(12) << sum_cv / double(blk)
        << setw(12) << this->error(sum_cv, sum_cv2, blk)
        << endl;
  coutf.close();
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  ...
  average  = _beta * _average(_index_chi);
  sum_average = _beta * _global_av(_index_chi);
  sum_ave2 = _beta * _beta * _global_av2(_index_chi);
  coutf << setw(12) << blk
        << setw(12) << average
        << setw(12) << sum_average/double(blk)
        << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
