...
void System :: initialize_velocities(){
  ...
  if(_restart){
  ...
  else {   // ESERCIZIO 4.2 — distribuzione delta

    double v_T = sqrt(3.0 * _temp);   
    vec vx(_npart), vy(_npart), vz(_npart);
    vx.zeros(); vy.zeros(); vz.zeros();

    vec sumv(_ndim, fill::zeros);

    // 1. Distribuzione delta
    for(int i=0; i<_npart; i++){
      int which = floor(_rnd.Rannyu(0,6));
      switch(which){
    ...
    // 4. Assegno velocità e calcolo x_old
    for(int i=0; i<_npart; i++){
...
void System :: print_positions(){
    cout << "PARTICLE 0: "
         << _particle(0).getposition(0,true) << " "
         << _particle(0).getposition(1,true) << " "
         << _particle(0).getposition(2,true) << endl;

    cout << "PARTICLE 1: "
         << _particle(1).getposition(0,true) << " "
         << _particle(1).getposition(1,true) << " "
         << _particle(1).getposition(2,true) << endl;
}

double System :: get_velocity(int i, int j){
  return _particle(i).getvelocity(j);
}

double System :: get_position(int i, int j){
  return _particle(i).getposition(j, true);
}

void System :: finale_state() {
  ofstream out;
  out.open("/home/luca_cutro/sim_num_es04/sim_num_es04.3/INPUT/final_state.dat");
  out << setprecision(10);
  for(int i=0; i<_npart; i++){
    out<<i<<" ";
    out<<_particle(i).getposition(0, true)<<" "
       <<_particle(i).getposition(1, true)<<" "
       <<_particle(i).getposition(2, true)<<" "
