...
Path::Path(int n_city) {
    for(int i=0; i<n_city; i++) {
        vec_path.push_back(i);
    }
}

Path :: Path(int n_city, Random& r){
    for(int i=0; i<n_city; i++) {
        vec_path.push_back(i);
    }
...
void Path :: random_swap(int n, Random& r){
    for(int i=0; i<n; i++){
        swapath(r);
    }
}

double Path :: length_city(const City& C, const City& D) const {
    return sqrt(pow((C.x - D.x), 2) + pow((C.y - D.y), 2));
}

double Path :: length_path(const Citys& C) const { 
    double sum = 0;
    int n = vec_path.size();
    for(int i=0; i< n; i++){
        int id1 = vec_path[i];
